...
namespace ProjectKillersServer.RequestHandlers {
    public class GetRoomsHandler {
        public static void DoHandle(NetDataRequest data, Client client, string networkID) {
            NetDataRequest response = new NetDataRequest(RequestTypes.EnterInMission, new Dictionary<string, ObjectWrapper>() { { "rooms", new ObjectWrapper<List<Room>>(Server.Rooms) } });
